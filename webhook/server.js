...
});content: 'Analise este comprovante/recibo de pagamento e retorne APENAS JSON valido. Texto: "' + analise + '". Retorne: {"valor":0.00,"categoria":"escolha UMA categoria: 👥 RH / Mão de Obra (se mencionar diaria/salario/freelancer/diarista/funcionario/trabalhador), 🥩 Matéria Prima (alimentos/insumos/carne/hortifruti/pao/produto), 🔧 Manutenção (conserto/reparo/tecnico), 💡 Energia / Utilidades (luz/agua/gas/conta), 🚚 Frete / Entregador (entrega/motoboy/frete/delivery/transporte), 🏢 Aluguel / Fixos (aluguel/iptu/condominio), 📦 Embalagem (caixa/sacola/embalagem), 🍺 Bebidas / Bar (so se mencionar bebida/cerveja/refri/drink/bar), 🧹 Limpeza / Higiene (limpeza/higiene/produto de limpeza), 💳 Taxas / Impostos (taxa/imposto/ifood/maquina), 📱 Telecom / Internet (internet/telefone/chip), 🎤 Shows / Eventos (musico/show/evento/DJ), 📣 Marketing (publicidade/marketing/impresso), ⚠️ Extravio / Perda (perda/descarte/estrago), 🔄 Outros (qualquer outro)","tipo":"pix|boleto|dinheiro|cartao","data":"DD/MM/YYYY (data que aparece no comprovante - NAO use a data de hoje)","descricao":"nome da EMPRESA ou PESSOA que RECEBEU o dinheiro. Se so aparecer Di Casa Gastronomia use o motivo do pagamento como descricao. NUNCA use Di Casa Gastronomia ou Di Casa como nome","destinatario":"mesmo que descricao"}. Se nao tiver valor claro retorne {"valor":0}';
nst http = require('http');
const https = require('https');
const PORT = process.env.PORT || 10000;
const EVO = 'https://evolution-api-latest-lrlv.onrender.com';
const EVO_KEY = 'dicasalaranjinha2024';
