    ...
    try {
      ...
      if (['imageMessage','documentMessage'].includes(tipo)) {
        ...
        const r1 = await claude([{ role:'user', content:[
          ...
          { type:'text', text:'Leia este comprovante de pagamento PIX. A empresa Di Casa Gastronomia PAGOU alguem. Identifique: (1) VALOR total pago, (2) NOME COMPLETO de quem RECEBEU - esse nome nao e Di Casa Gastronomia, procure APENAS nos campos Favorecido/Para/Recebedor/Beneficiario/Nome da conta destino - IGNORE campos de observacao/descricao/motivo, (3) DATA do comprovante, (4) TIPO pix/boleto/cartao, (5) OBSERVACAO/MOTIVO do pagamento se houver no campo descricao/observacao do recibo - ex: diarias, entrega, material. Se nao tiver observacao informe SEM_DESCRICAO. IMPORTANTE: nome do recebedor e observacao sao campos DIFERENTES. Liste cada campo claramente em portugues.' }
        ]}], 800);
        const analise = r1.content && r1.content[0] ? r1.content[0].text : 'Nao consegui extrair.';
        console.log('Analise:', analise.substring(0,100));
        const prompt2 = 'Extraia do texto abaixo APENAS JSON valido. Texto: "' + analise + '". Formato: {"valor":0.00,"destinatario":"NOME COMPLETO da conta/pessoa que recebeu - campo Favorecido/Beneficiario/Nome da conta destino - NAO usar observacao/descricao/motivo do pix","categoria":"🥩 Matéria Prima|👥 RH / Mão de Obra|🔧 Manutenção|💡 Energia / Utilidades|🚚 Frete / Entregador|🏢 Aluguel / Fixos|📦 Embalagem|🍺 Bebidas / Bar|🧹 Limpeza / Higiene|💳 Taxas / Impostos|📱 Telecom / Internet|🔄 Outros","tipo":"pix|boleto|dinheiro|cartao","data":"DD/MM/AAAA","descricao":"motivo do pagamento se houver"}. Se nao tiver valor retorne {"valor":0}';
