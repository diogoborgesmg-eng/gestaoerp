...
module.exports = async function handler(req, res) {
  ...
  if (path === '/api/nfe/sefaz' && req.method === 'POST') {
    ...
    if (!cnpj || !pfxBase64) return res.status(400).json({ erro: 'CNPJ e certificado obrigatórios' });
    ...
    try {
      ...
      try {
        ...
        return res.status(200).json({ ok: false, erro: 'Senha incorreta ou certificado inválido: ' + eCert.message });
      }
      
      if (!privateKey || !certificate) {
        return res.status(200).json({ ok: false, erro: 'Não foi possível extrair chave do certificado' });
      }
      ...
      const soapBody = '<?xml version="1.0" encoding="UTF-8"?>' +
        ...
        '<soapenv:Header/><soapenv:Body><nfeDist:nfeDistDFeInteresse><nfeDist:nfeDadosMsg>' +
        '<distDFeInt versao="1.01" xmlns="http://www.portalfiscal.inf.br/nfe">' +
        '<tpAmb>1</tpAmb><cUFAutor>31</cUFAutor><CNPJ>' + cnpjLimpo + '</CNPJ>' +
      ...
      const sefazResp = await new Promise((resolve, reject) => {
        ...
        const r = https.request(opts, res2 => {
          let d = '';
          res2.on('data', c => d += c);
          res2.on('end', () => resolve({ status: res2.statusCode, data: d }));
        });
        r.on('error', reject);
        r.on('timeout', () => { r.destroy(); reject(new Error('Timeout SEFAZ')); });
        r.write(soapBody);
        r.end();
      });
      
      if (sefazResp.status !== 200) {
        return res.status(200).json({ ok: false, erro: 'SEFAZ HTTP ' + sefazResp.status, raw: sefazResp.data.substring(0, 300) });
      }
      
      const xml = sefazResp.data;
      ...
      // Extrai documentos
      const nfs = [];
      const docMatches = [...xml.matchAll(/docZip[^>]*>([^<]+)<\/docZip/g)];
      for (const m of docMatches) {
        try {
          const xmlDoc = Buffer.from(m[1], 'base64').toString('utf8');
          const decompressed = xmlDoc;
          const chave = (decompressed.match(/chNFe>([^<]+)/) || [])[1] || '';
          if (!chave) continue;
          nfs.push({
            id: chave, chave,
            numero: (decompressed.match(/nNF>([^<]+)/) || [])[1] || '',
            serie: (decompressed.match(/serie>([^<]+)/) || [])[1] || '',
            emitente: (decompressed.match(/xNome>([^<]+)/) || [])[1] || 'Fornecedor',
            emitCNPJ: (decompressed.match(/CNPJ>([^<]+)/) || [])[1] || '',
            valor: parseFloat((decompressed.match(/vNF>([^<]+)/) || [])[1] || '0'),
            data: ((decompressed.match(/dhEmi>([^<]+)/) || [])[1] || '').substring(0,10),
            status: 'pendente'
          });
        } catch(ep) {}
      }
